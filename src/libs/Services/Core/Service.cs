...
namespace svc
{
	...
	*/
	...
	public class CerasSerializerForShielded : ISerializer
	{
		...
		static CerasSerializerForShielded()
		{
			s_config = new SerializerConfig();
			s_config.Advanced.PersistTypeCache = true;
		}

		readonly ThreadLocal<CerasSerializer> m_sendCeras = new ThreadLocal<CerasSerializer>( () => new CerasSerializer( s_config ) );
		readonly ThreadLocal<CerasSerializer> m_recvCeras = new ThreadLocal<CerasSerializer>( () => new CerasSerializer( s_config ) );

		public CerasSerializerForShielded()
		{
		}
		...
		{
			var recv = m_recvCeras.Value;

			var obj = recv.Deserialize<object>( bytes );
		...
		{
			var send = m_sendCeras.Value;

			var buf = send.Serialize<object>( obj );
